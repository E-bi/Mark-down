...
* 应用场景
  ...
  ```python
  1. 使用Redis来缓存一些经常被用到、或者需要消耗大量资源的内容, 通过这些内容放到redis里面, 程序可以快速读取这些内容
  2. 一个网站, 如果某个页面经常会被访问到, 或者常见页面时消耗的资源比较多, 比如需要多次访问数据库、生成的时间比较长等, 我们可以使用redis将这个页面缓存起来, 减轻网站负担, 降低网站的延迟, 比如网站首页等
  # redis的诞生是为了解决负载问题
  ```
...
* redis附加功能
  ...
  ```python
  1. 持久化
  	 将内存中数据保存在磁盘中, 保证数据安全, 方便进行数据备份和恢复
  2. 过期键功能
...
# 客户端连接
redis-cli -h IP地址(127.0.0.1) -p 端口(6379) -a 密码
...
# 4. 设置过期时间(ex)
set key value ex seconds # set key value ex 3
