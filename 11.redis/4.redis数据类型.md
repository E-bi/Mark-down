...
- **优点**
  ...
  ![image-20210316155827709](../static/4.redis%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B/image-20210316155827709.png)
...
- **代码实现**
  ...
  class Update(object):
      ...
      def update_mysql(self, score, username):
          ...
          try:
              self.cur.execute(upd, [score, username])
              self.db.commit()
              return True
      ...
      def run(self):
          ...
          # mysql成功后更新redis
          if self.update_mysql(score,username):
...
r.sadd('user10', '佩奇', '乔治', '丹尼')
r.sadd('user11', '佩奇', '大天使', '李珂')

print(r.sinter('user10', 'user11'))
```
...
# zrangebyscore student:salary 0 10000 limit 0 1
...
# zunionstore salary3 2 salary1 salary2 weights 1 0.5 AGGREGATE MAX
# 2代表集合数量,weights之后 权重1给salary,权重0.5给salary2集合,算完权重之后执行聚合AGGREGATE
                     
# 交集：和并集类似，只取相同的元素
zinterstore aaa 2 key1 key2
...
r.zadd('fruit:price', {'apple': 3, 'mongo': 10, 'bnana': 2, 'cherry': 50})
#
print(r.zrange('fruit:price', 0, -1, withscores=True))
print(r.zrangebyscore('fruit:price', 5, 10, start=0, num=2, withscores=True))
r.zrem('fruit:price', 'apple')
r.zincrby('fruit:price', 2, 'mongo')

r.zadd('fruit:price2', {'orange': 8, 'nut': 20})
# 并
r.zunionstore('fruit:union', ('fruit:price', 'fruit:price2'), aggregate='max')
r.zrange('fruit:union', 3, 100, withscores=True)
...
1、每首歌的歌名作为元素
2、每首歌的播放次数作为分值
3、使用ZREVRANGE来获取播放次数最多的前3歌曲
import redis

r = redis.Redis(host='127.0.0.1', port=6379, db=0)
r.zadd('rank', {'huahai': 100, 'bigbang': 89, 'laorenyuhai': 77, '挪威的深林': 50}, nx=True)

# 增加播放次数
r.zincrby('rank', 20, 'huahai')

res = r.zrevrange('rank', 0, 2, withscores=True)
print(res)
for i, name in enumerate(res):
    print('第{}名:{} 播放次数:{}'.format(i,name[0],int(name[1])))
```
...
# 方案1
arange mobile-003 0 -1 withscores=True
# 方案2
zunionstore mobile-001:003 3 mobile-001 mobile-002 mobile-003 aggregate max
```
...
# 1.并集
r.zunionstore('mobile-001:003', ('mobile-001', 'mobile-002', 'mobile-003'), aggregate='sum')
res = r.zrevrange('mobile-001:003', 0, 2, withscores=True)
for i,item in enumerate(res):
    print('第一名{}: 销量:{}'.format(item[0], item[1]))
```
